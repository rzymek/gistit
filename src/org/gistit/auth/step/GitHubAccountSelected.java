...
public class GitHubAccountSelected extends SetupItem {
	...
	private final UIAction RUN_GITHUB_APP = new UIAction() {
		...
		public void run(Activity parent) {
			...
			if (github == null) {
				ask("Install GitHub ",
						"GistIt uses GitHub accounts. Please install GitHub application to configue one.",
						new OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog, int which) {
								...
								try {
									...
									context.app.startActivity(new Intent(Intent.ACTION_VIEW, uri));
				...
				ask("Setup GitHub account", "Please sign in using GitHub application.", new OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						dialog.dismiss();
						context.app.startActivity(github);
					}
				});
			}
		}
	...
	private void ask(CharSequence title, CharSequence msg, DialogInterface.OnClickListener onConfirm) {
		AlertDialog.Builder builder = new AlertDialog.Builder(context.app);
